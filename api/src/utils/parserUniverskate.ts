...
export const changeColor = (originColor: string): string | undefined =>
  colorMap[originColor];

export const changeSize = (originSize: string): string | undefined =>
  sizeMap[originSize];
...
export function extractCSizes(reference: string): string | undefined {
...
export function extractColor(reference: string | undefined | null): string | undefined {
  if (!reference || typeof reference !== 'string') return undefined;

  const parts = reference.split('-').reverse(); // check from end to start
