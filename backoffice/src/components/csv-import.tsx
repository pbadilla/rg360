...
export const CSVImport = () => {
  ...
  const [stats, setStats] = useState<{ total: number, succeeded: number, failed: number }>({ 
      ...
      const results = await Promise.all(
        products.map(async (product) => {
          try {
            ...
            if (!product.name || isNaN(product.price) || isNaN(product.stock) || !product.category) {
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-6">
        <div className="flex flex-col items-center justify-center border-2 border-dashed border-gray-300 rounded-lg p-6 hover:border-primary transition-colors">
          ...
          >
            ...
            <span className="text-sm text-muted-foreground mt-1">
              Your CSV must include columns for: name, price, stock, and category
            </span>
          </label>
        </div>
        ...
        {stats.total > 0 && (
          <div className="flex flex-col gap-2 p-3 bg-gray-50 rounded-md text-sm">
