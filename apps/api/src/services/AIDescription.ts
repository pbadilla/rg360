...
// Utility wrapper to collect errors but not spam console
async function safeRun<T>(name: string, fn: () => Promise<T>): Promise<T> {
  try {
    return await fn();
  } catch (err: any) {
    errorLog.push(`❌ ${name} failed: ${err?.message || err}`);
    throw err;
  }
}
    ...
    try {
      return await safeRun(`HuggingFace(${hfModel})`, async () => {
        const res = await fetch(
          `https://api-inference.huggingface.co/models/${hfModel}?wait_for_model=true`,
          {
            method: "POST",
            headers: {
...
export async function AIDescription(product: ProductForDescription): Promise<string> {
  const providers = [
    tryOpenAI,     // prefer OpenAI first if available
    tryCohere,
    tryHuggingFace,
    tryAI21,
    tryDeepAI,
