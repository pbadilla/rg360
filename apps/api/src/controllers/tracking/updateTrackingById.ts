...
import { PromotionsModel } from '@/models/promotions'; // Asume que tienes este modelo

const updatePromotionsById = async (req: Request, res: Response, _next: NextFunction) => {
  console.log('updatePromotionsById called', req.params, req.body);
  
  const { id } = req.params;
  
  if (!id) {
    return res.status(400).json({ message: 'Promotions ID is required.' });
  }
  
  console.log('Promotions ID:', id);
  ...
  try {
    const promotions = await PromotionsModel.findByIdAndUpdate(
      id,
      updateData,
      { 
    ...
    if (error.name === 'CastError') {
      return res.status(400).json({ 
        message: 'Invalid promotions ID format.' 
      });
    }
...
export default updatePromotionsById;